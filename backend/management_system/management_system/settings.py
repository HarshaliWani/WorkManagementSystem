...
if ALLOWED_HOSTS_ENV:
    ...
    ALLOWED_HOSTS = ['localhost', '127.0.0.1','13.51.206.221']
...
CORS_ALLOWED_ORIGINS = [
    ...
    "http://localhost:3000",
    "http://localhost:8000",
    "http://13.51.206.221:3000",
    "http://13.51.206.221:8000",
]

CORS_ALLOW_CREDENTIALS = True

CSRF_TRUSTED_ORIGINS = [
    "http://13.51.206.221:8000",  # Backend
    "http://13.51.206.221:3000",  # Frontend (if different port)
    "http://localhost:8000",
]
...
# Basic security headers (work with HTTP and HTTPS)
SECURE_BROWSER_XSS_FILTER = True
SECURE_CONTENT_TYPE_NOSNIFF = True
X_FRAME_OPTIONS = 'SAMEORIGIN'  # Allow same origin (needed for admin)

# CSRF Configuration
CSRF_COOKIE_SECURE = USE_HTTPS  # True only with HTTPS
CSRF_COOKIE_HTTPONLY = False  # Must be False for Django admin
CSRF_COOKIE_SAMESITE = 'Lax'
CSRF_USE_SESSIONS = False  # Store CSRF token in cookie, not session

# Session Configuration  
SESSION_COOKIE_SECURE = USE_HTTPS  # True only with HTTPS
SESSION_COOKIE_HTTPONLY = True  # Prevent JavaScript access
SESSION_COOKIE_SAMESITE = 'Lax'
SESSION_ENGINE = 'django.contrib.sessions.backends.db'

# HTTPS-only security features (only enable when USE_HTTPS=True)
if USE_HTTPS:
    ...
    # HTTP mode - disable HTTPS-only features
